  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const enhance = (resList, type) => resList.map(res => {
        ...
        return {
          ...res,
          display_name: res.type_name || wasteInfo.type_name || 'Geri Dönüşüm Atığı',
          display_desc: res.waste_description || wasteInfo.description || 'Açıklama belirtilmedi',
          display_amount: res.amount || wasteInfo.amount || '0',
          display_unit: res.official_unit || wasteInfo.official_unit || 'kg',
          
          city: res.city || wasteInfo.city || 'Belirtilmedi',
          district: res.district || wasteInfo.district || '',
          neighborhood: res.neighborhood || wasteInfo.neighborhood || '',
          street: res.street || wasteInfo.street || 'Adres bilgisi mevcut değil',
          address_details: res.address_details || wasteInfo.address_details || 'Ek detay yok',
          
          contact_label: type === 'collector' ? "Atık Sahibi" : "Toplayıcı Bilgisi",
          contact_name: type === 'collector' ? (res.owner_name || 'İsimsiz Kullanıcı') : (res.collector_name || 'Atanmış Toplayıcı'),
                    ...
                    <div key={item.reservation_id} className="flex justify-between items-center p-6 border border-slate-100 rounded-3xl bg-white hover:border-emerald-200 transition-all">
                      <div className="flex gap-4 items-center">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-bold text-slate-400 uppercase tracking-wide">{item.display_amount} {item.display_unit} • {item.status}</p>
                        </div>
                      </div>
                      <button onClick={() => { setSelectedItem(item); setEditAmount(item.display_amount); }} className="btn btn-sm btn-ghost bg-slate-100 font-bold px-6 rounded-xl">İncele</button>
                    </div>
                  ))}
                  {(activeTab === 'collector' ? toCollect : myResv).length === 0 && <p className="text-center py-10 font-bold text-slate-300">Henüz kayıt yok.</p>}
                </div>
              )}
            </div>
          </div>
